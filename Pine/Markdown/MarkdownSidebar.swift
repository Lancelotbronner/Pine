...
struct MarkdownSidebar: View {
	let document: MarkdownDocument

	var body: some View {
		SidebarViewControllerRepresentable(document: document)
	}
}

private struct SidebarViewControllerRepresentable: NSViewControllerRepresentable {
	let document: MarkdownDocument

	func makeNSViewController(context: Context) -> NSSidebarViewController {
		NSSidebarViewController(document: document, environment: context.environment)
	}

	func updateNSViewController(_ nsViewController: NSSidebarViewController, context: Context) {

	}
}

private final class NSSidebarViewController: NSViewController {

	@IBOutlet weak private var sidebar: NSOutlineView!
	@IBOutlet weak private var sidebarActionsView: NSView!

	private let document: MarkdownDocument
	private let documentConfiguration: DocumentConfiguration?
	private let openDocument: OpenDocumentAction
	private let sidebarSynchronizer: SidebarSynchronizer

	private var onSynchronizeSidebar: AnyCancellable?
	...
	init(document: MarkdownDocument, environment: EnvironmentValues) {
		self.document = document
		self.documentConfiguration = environment.documentConfiguration
		self.openDocument = environment.openDocument
		self.sidebarSynchronizer = environment.application.sidebar
		super.init(nibName: "MarkdownSidebar", bundle: .main)
		onSynchronizeSidebar = environment.application.sidebar.objectWillChange
	...
	private func syncSelectedRow() {
		...
		for row in IndexSet(integersIn: 0..<sidebar.numberOfRows) {
			...
			if rowItem?.fullPath == documentConfiguration?.fileURL?.relativePath {
				selectedRow = row
			}
		}
	...
	@objc private func removeItemFromSidebar(_ sender: NSMenuItem) {
		...
		sidebarSynchronizer.objectWillChange.send()
	}
...
extension NSSidebarViewController: NSOutlineViewDataSource {
	...
	func outlineViewSelectionDidChange(_ notification: Notification) {
		...
		Task {
			try await openDocument(at: doc.fileURL)
		}
	}
...
extension NSSidebarViewController: NSOutlineViewDelegate {
...
extension NSSidebarViewController: NSMenuDelegate {
